#!/usr/bin/env python3
"""Gate — OpenAPI api-pac-context expõe só allowlist investigativa (≤30 operações ChatGPT)."""
...
def validate(schema: dict) -> dict:
    ...
    extra = sorted(published - ANALYST_CONTEXT_OPERATION_IDS)
    missing = sorted(ANALYST_CONTEXT_OPERATION_IDS - published)
    ok = (
        not extra
        and not missing
        and len(published) == len(ANALYST_CONTEXT_OPERATION_IDS)
    ...
    return {
        ...
        "expectedCount": len(ANALYST_CONTEXT_OPERATION_IDS),
...
def main() -> int:
    ...
    parser.add_argument(
        "--openapi-url",
        default=os.environ.get("CTX_OPENAPI_URL", ""),
